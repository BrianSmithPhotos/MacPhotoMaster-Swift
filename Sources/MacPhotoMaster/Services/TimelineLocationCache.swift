...
actor TimelineLocationCache {
    static let defaultMaxMatchSeconds = 30 * 60

    private let dbQueue: DatabaseQueue
    private let maxMatchSeconds: Int

    init(databasePath: URL, maxMatchSeconds: Int = TimelineLocationCache.defaultMaxMatchSeconds) throws {
    ...
    func suggestion(forCaptureTimestampUTC captureTimestampUTC: Int) throws -> GPSSuggestion? {
        try dbQueue.read { db in
            guard
